...
class resnet(_fasterRCNN):
  ...
  def _init_modules(self):
    ...
    self.Encoder_Content.cuda()

    self.netD_ds = netD()

    self.RCNN_top_base = nn.Sequential(resnet.layer4)
    self.RCNN_top_di = copy.deepcopy(nn.Sequential(resnet.layer4))
    #self.RCNN_top_content = nn.Sequential(resnet.layer4)

    self.RCNN_cls_score_base = nn.Linear(2048, self.n_classes)
    self.RCNN_cls_score_di = nn.Linear(2048, self.n_classes)
    #self.RCNN_cls_score_content = nn.Linear(2048, self.n_classes)
    if self.class_agnostic:
      self.RCNN_bbox_pred_base = nn.Linear(2048, 4)
      self.RCNN_bbox_pred_di = nn.Linear(2048, 4)
      #self.RCNN_bbox_pred_content = nn.Linear(2048, 4)
    else:
      self.RCNN_bbox_pred_base = nn.Linear(2048, 4 * self.n_classes)
      self.RCNN_bbox_pred_di = nn.Linear(2048, 4 * self.n_classes)
    ...
    # self.RCNN_top_content.apply(set_bn_fix)
